...
2. Every ever-member of the S&P 500 since the history start is reconstructed
   ...
   (`interval="1mo"`), inheriting all of its gates.
3. Members are grouped into **companies** before aggregation (below), then
   each month-end label is aggregated independently.
4. Quality gate: months with fewer than `PIPELINE_MIN_SP500_SIZE` companies or
   ...
   gaps with a recorded reason; if fewer than
   `INDEX_VALUATION_MIN_MONTH_COVERAGE` of all months are valid, the run
...
- **Multi-class members are one company.** Grouping uses the cached official
  ...
  with a documented fallback list (`STATIC_COMPANY_GROUPS`) for delisted pairs
  the SEC file no longer lists. Company equity sums the classes; company-wide
  earnings are counted exactly once. When two classes report earnings that
  disagree beyond a 2% tolerance, the company stays a gap for that month
  rather than guessing which class is right.
...
Per ever-member it backfills prices over that member's own membership window
(one EOD call), fetches full fundamentals once when stored statements do not
cover the window (ten calls; current screener members already carry full
statement history), and verifies complete split history (one call) — roughly
800 price calls + 800 split calls + ~10 calls per still-fundamentals-less
member in total. Completed tickers are skipped on re-run. The migration
(`0021_index_valuation_snapshots`) runs with the normal `alembic upgrade head`.
