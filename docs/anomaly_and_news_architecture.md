...
### 2.4 定时纪要渲染与审计
* **证据保真**：开盘及盘后纪要直接渲染异常扫描中已经保存的 `ai_analysis`、`attribution_status` 和编号新闻链接，不再把裸行情交给第二次 LLM 调用重新猜测驱动。
* **失败闭合**：没有新闻或归因失败时只展示明确的降级说明，不补写基本面原因，也不自动生成交易建议。
* **投递审计**：`daily_report_runs` 保存最终正文、输入的扫描结果、渲染器版本和通知投递状态，支持事后回放。

### 2.5 API 路由扩展 (Routers)
