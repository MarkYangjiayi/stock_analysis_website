                ...
                <ul className="divide-y divide-[var(--border)]">
                    {current.matches.map(item => {
                        ...
                        return <li key={item.ticker} className="grid grid-cols-[minmax(0,1fr)_auto] items-center gap-x-4 gap-y-2 py-3 sm:grid-cols-[minmax(0,1fr)_minmax(120px,180px)_80px_auto]">
                            ...
                            <div className="col-start-2 row-start-1 flex items-center gap-2 sm:col-auto sm:row-auto"><button className="secondary-button w-24 shrink-0 justify-center px-2 py-1 text-xs" aria-expanded={selected === item.ticker} aria-controls="similar-comparison" onClick={() => setSelected(selected === item.ticker ? null : item.ticker)}>{selected === item.ticker ? <>Close <ChevronUp size={13} /></> : <>Compare <ChevronDown size={13} /></>}</button>
