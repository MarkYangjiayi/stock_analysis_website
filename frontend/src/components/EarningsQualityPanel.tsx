                        ...
                        <article key={key} className="overflow-hidden rounded-xl border">
                            ...
                            {isExpanded && <div className="space-y-4 border-t p-4">
                                ...
                                {analysis?.status === "completed" && analysis.result && <div className="space-y-3 rounded-lg border p-3"><div className="flex flex-wrap items-center justify-between gap-2"><p className="text-xs font-black">Filing extraction</p><span className={`rounded-full px-2 py-0.5 text-[10px] font-black uppercase ${analysis.result.verification_status === "verified" ? "bg-emerald-100 text-emerald-700 dark:bg-emerald-950 dark:text-emerald-300" : "bg-slate-100 text-slate-600 dark:bg-slate-800 dark:text-slate-300"}`}>{analysis.result.verification_status === "verified" ? "Verified" : "Shadow / flag only"}</span></div>
                                    ...
                                    {analysis.result.adjustments.length ? <div className="space-y-2">{analysis.result.adjustments.map((adjustment, index) => <div key={`${adjustment.label}-${index}`} className="rounded-lg bg-slate-50 p-3 text-xs dark:bg-slate-900/50"><div className="flex flex-wrap justify-between gap-2"><span className="font-bold">{adjustment.label}</span><span className="font-mono">After tax {compact(adjustment.earnings_effect_after_tax)}</span></div><div className="mt-2 grid grid-cols-3 gap-2 font-mono text-[10px]"><span>Pre-tax {compact(adjustment.pretax_earnings_effect)}</span><span>Tax {compact(adjustment.tax_effect)}</span><span>After-tax {compact(adjustment.earnings_effect_after_tax)}</span></div><p className="mt-1 text-slate-500">{adjustment.category.replaceAll("_", " ")} · {adjustment.cash_effect.replace("_", " ")} · {adjustment.include_in_normalized ? "included if verified" : "flag only"}</p><blockquote className="mt-2 border-l-2 pl-2 text-[11px] leading-4 text-slate-500">{adjustment.citation.excerpt}</blockquote><p className="mt-2 font-mono text-[10px] text-slate-400">{adjustment.citation.accession} · {adjustment.citation.document_name} · {adjustment.citation.section} · source amount {compact(adjustment.citation.source_amount)} × {compact(adjustment.citation.source_unit_scale)}</p></div>)}</div> : rejectedAdjustments.length || unquantifiedCandidates.length ? <p className="text-xs text-slate-500">No source-validated adjustment amount is available.</p> : <p className="text-xs text-slate-500">No adjustment item was extracted.</p>}
                                    {unquantifiedCandidates.length > 0 && <div className="rounded-lg border border-amber-200 bg-amber-50/70 p-3 text-xs text-amber-900 dark:border-amber-900 dark:bg-amber-950/20 dark:text-amber-300"><p className="font-black">Unquantified filing candidates</p><p className="mt-1 leading-5">These filing items remain visible as red flags, but no finite, non-zero earnings effect was available for amount validation.</p><div className="mt-2 space-y-2">{unquantifiedCandidates.map((candidate, index) => <div key={`${index}-${candidate.label}`} className="rounded-md border border-amber-200/70 p-2 dark:border-amber-900"><p className="font-bold">{candidate.label}</p><p className="mt-1 text-[10px] text-amber-800/80 dark:text-amber-300/80">{candidate.policy_category.replaceAll("_", " ")} · flag only · amount unavailable</p></div>)}</div></div>}
                                    {rejectedAdjustments.length > 0 && <div className="rounded-lg border border-amber-200 bg-amber-50/70 p-3 text-xs text-amber-900 dark:border-amber-900 dark:bg-amber-950/20 dark:text-amber-300"><p className="font-black">Unverified filing candidates</p><p className="mt-1 leading-5">Candidate names and categories are shown as red flags; amounts are hidden because source, period, or amount validation did not pass.</p><div className="mt-2 space-y-2">{rejectedAdjustments.map((candidate) => <div key={`${candidate.adjustment_index}-${candidate.label}`} className="rounded-md border border-amber-200/70 p-2 dark:border-amber-900"><p className="font-bold">{candidate.label}</p><p className="mt-1 text-[10px] text-amber-800/80 dark:text-amber-300/80">{candidate.policy_category.replaceAll("_", " ")} · flag only</p><p className="mt-1 font-mono text-[10px] text-amber-800/70 dark:text-amber-300/70">{candidate.failure_codes?.length ? candidate.failure_codes.join(" · ") : "source_or_period_validation_failed"}</p></div>)}</div></div>}
