                    ...
                    return (
                        <article key={key} className="overflow-hidden rounded-xl border">
                            ...
                            {isExpanded && <div className="space-y-4 border-t p-4">
                                ...
                                {(!analysis || analysis.status === "failed") && <div className="flex flex-col gap-2 border-t pt-3 sm:flex-row sm:items-center sm:justify-between"><p className="text-xs leading-5 text-slate-500">Runs only for this period after you click. It is never part of sync, scheduling, or batch work.</p>{!data.sec_analysis.supported ? <span className="text-xs font-bold text-slate-500">{data.sec_analysis.reason}</span> : <button type="button" disabled={isBusy} onClick={() => adminKey ? void onAnalyze?.(period) : onUnlock?.()} className="primary-button shrink-0"><span>{adminKey ? <FileSearch size={15} /> : <KeyRound size={15} />}</span>{adminKey ? (analysis?.status === "failed" ? "Retry filing analysis" : "Analyze filing") : "Unlock to analyze"}</button>}</div>}
                            </div>}
                        </article>
                    );
                })}
